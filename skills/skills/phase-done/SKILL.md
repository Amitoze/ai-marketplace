...
0. **Verify the tree is committed FIRST**: `git status` must be clean
   ...
   claims (this has bitten before: a verified file was left uncommitted
   and the PR merged without it). If the tree is dirty, commit the work
   before anything else.
1. **Verify the gate**: find the phase's GATE section in the relevant
   `plan/` file and actually check each condition (run the tests or
   commands that prove it). If a gate item requires a BY-EYE or human
   check, present the material, say exactly what to look for, and ask
   the user to confirm each item — **never self-certify a human check**.
   If the gate fails, stop and report; the phase is not done.
2. **Run /plan-sync** (checkboxes, status table, DECISIONS.md prompts).
3. **Commit** any outstanding work — message includes measured or
   observed results where available, and any tuned config values that
   produced them.
4. **Push** the branch.
5. **PR** (if the project is using branches; if working trunk-style on
   ...
   "## For the reviewer" with the gate outcomes, measured results, and
   anything needing judgement. Title: "Phase <X>: <summary>". End with
