...
2. **Verify the merge actually happened** before deleting anything:
   ...
   **This verification must be its own command, and its output must be
   ...
   (Violated once, 2026-07-23: the check printed OPEN, the chain
   deleted the branch under the open PR anyway; recovered via local
   commit + reopen. Also beware: `gh pr view <branch>` returns the
   branch's MOST RECENT PR — after a follow-up PR exists, an earlier
   merged PR on the same branch does not mean the branch is merged.)
...
5. **Cut the next branch**: `phase-<letter>-<short-name>` where the
   short name comes from the phase's plan title (e.g. Phase C "OCR
   Module" → `phase-c-ocr`). Push with `-u origin`.
6. **Orient**: read the next phase's section in its `plan/` file and
   ...
   its GATE will require (and, if the gate is the user's own judgement,
   what they will be asked to judge).
...
- If the project is being worked trunk-style (commits straight to main,
  no PRs), say so and offer the lightweight version: skip steps 2–4,
  just confirm main is clean and orient on the next phase. Do not
  manufacture a branch dance the project doesn't use.
