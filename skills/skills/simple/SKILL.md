...
1. **Plain words first.** No jargon. If a technical term is unavoidable,
   define it descriptively at first use — "a tokenizer — the tool that chops
   text into the little pieces a model actually reads". Reach for an everyday
   analogy (an edition of a book, a library card, a fingerprint) over
   precision.
2. **One diagram.** Include a small plain-text (ASCII) diagram whenever the
   ...
   flows, in plain words ("the trust score", "which folder"), never a type
   name. Mark what is new or changed if it matters. Follow the diagram with a
   two-or-three-sentence prose reading of it, walking the path in order.
3. **Short.** A few tight paragraphs, not an essay. Aim for the "click", not
   completeness — depth is available on request.
4. **Ground it in their case.** Use the actual example at hand (this book,
   this chunk, this decision), not an abstract one.
...
- If the topic is sensitive or high-stakes, keep the tone calm and the
  analogies boring; never trivialise.
